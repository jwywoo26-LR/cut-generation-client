...
export default function EnhancedImageGeneration({ currentTable, onImagesGenerated }: EnhancedImageGenerationProps) {
  ...
  const handleGenerateImages = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to generate enhanced images');
      }
      ...
      if (data.processedCount === 0) {
        setError('No records found that need enhanced image generation');
      } else {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <div className="space-y-4">
          <p className="text-gray-600 dark:text-gray-300 text-sm">
            Generate images for each record using the enhanced_prompt and reference_image_attached.
            Images will be saved to enhanced_prompt_image fields (1, 2, 3).
            {currentTable && (
          ...
          <div className="space-y-3">
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
              <div className="text-blue-800 dark:text-blue-200 text-sm">
                <strong>Dynamic Columns:</strong> The system automatically creates Airtable fields like enhanced_prompt_image_1, enhanced_prompt_image_2, etc., based on your selected count. No manual column setup needed!
              </div>
            </div>
            
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
              <div className="text-yellow-800 dark:text-yellow-200 text-sm">
                <strong>Note:</strong> Enhanced image generation can take several minutes per image. 
                The process will generate {useCustomCount ? parseInt(customCount) || 1 : imageCount} image{(useCustomCount ? parseInt(customCount) || 1 : imageCount) > 1 ? 's' : ''} per record using your selected model and reference images with enhanced prompts.
              </div>
            </div>
          </div>
